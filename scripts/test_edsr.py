...
def evaluate_dataset(model, device, csv_file, dataset_name):
    ...
    output_dir = project_root / "results" / "edsr_lite" / dataset_name
    ...
    for index, row in enumerate(tqdm(rows, desc=f"EDSR-lite {dataset_name}")):
        ...
        sr_tensor, inference_time = measure_inference(model, lr_tensor, device)
        ...
        sr_img.save(output_dir / f"{image_name}_edsr_lite.png")
...
def load_model(device, scale=4):
    checkpoint_path = (
        ...
        / "results"
        / "checkpoints"
        / f"edsr_lite_x{scale}_best.pth"
    )
    if not checkpoint_path.exists():
        raise FileNotFoundError(
            f"Checkpoint not found: {checkpoint_path}. Train EDSR-lite first."
        )
...
def main():
    ...
    for dataset_name, csv_file in test_sets:
        metric_rows, summary = evaluate_dataset(
            model, device, csv_file, dataset_name
        )
        all_metric_rows.extend(metric_rows)
        summary_rows.append(summary)

    metrics_dir = project_root / "results" / "metrics"
    details_path = metrics_dir / "edsr_lite_x4_details.csv"
    summary_path = metrics_dir / "edsr_lite_x4_summary.csv"
    write_csv(
        details_path,
        all_metric_rows,
    ...
    print("=" * 60)
    print("EDSR-lite testing completed.")
